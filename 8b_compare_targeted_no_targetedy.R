...
#' Compare SMARCB1 binding profiles between targeted and non-targeted genes
#' 
#' Input files:
...
#' - Gene_lists/targets/high_expression_targets2_1000.0.csv: List of targeted genes
#' - Gene_lists/targets/high_expression_no_targets_1000.0.csv: List of non-targeted genes
#'
#' Output files:
#' - results/metaprofiles_comparison_R/combined_targeted_vs_no_targeted_profile.pdf: 
...
#' targeted and non-targeted genes, generating metaprofiles and fold change plots.
...
main <- function() {
  ...
  # Define gene lists
  targeted_genes <- "Gene_lists/targets/high_expression_targets2_1000.0.csv"
  no_targeted_genes <- "Gene_lists/targets/high_expression_no_targets_1000.0.csv"
  
  # Create output directory
  dir.create("results/metaprofiles_comparison", showWarnings = FALSE)
  ...
  # Process gene lists
  cat("Processing targeted genes...\n")
  targeted_regions <- create_tss_regions(targeted_genes, gtf_file)
  cat("Processing no targeted genes...\n")
  no_targeted_regions <- create_tss_regions(no_targeted_genes, gtf_file)
  
  # Calculate profiles
  targeted_bg_profile <- calculate_metaprofile(bg_data, targeted_regions)
  targeted_bm_profile <- calculate_metaprofile(bm_data, targeted_regions)
  no_targeted_bg_profile <- calculate_metaprofile(bg_data, no_targeted_regions)
  ...
  plot_data <- rbind(
    create_plot_data(targeted_bg_profile, targeted_bm_profile, "Targeted"),
    create_plot_data(no_targeted_bg_profile, no_targeted_bm_profile, "No Targeted")
  )
  
  # Calculate fold changes
  targeted_fc <- calculate_fold_change(targeted_bg_profile, targeted_bm_profile)
  no_targeted_fc <- calculate_fold_change(no_targeted_bg_profile, no_targeted_bm_profile)
  
  fc_data <- data.frame(
    Position = rep(x_pos, 2),
    FoldChange = c(targeted_fc, no_targeted_fc),
  ...
  p1 <- ggplot(plot_data, aes(x = Position, y = Signal, color = Condition, linetype = Group)) +
    ...
    labs(title = "SMARCB1 binding around TSS",
  ...
  p2 <- ggplot(fc_data, aes(x = Position, y = FoldChange, color = Group)) +
    ...
    scale_color_manual(values = c("Targeted" = "#ff7f0e", "No Targeted" = "#2ca02c")) +
  ...
  # Save plots
  ggsave(
    filename = "results/metaprofiles_comparison_R/combined_targeted_vs_no_targeted_profile.pdf",
    plot = combined_plot,
    width = 10,
    height = 12
  )
  ...
  cat("\nSummary Statistics:\n")
  cat("\nTargeted Genes:\n")
  cat("Mean BG signal:", mean(colMeans(targeted_bg_profile)), "\n")
  cat("Mean BM signal:", mean(colMeans(targeted_bm_profile)), "\n")
  cat("Mean log2 fold change:", mean(targeted_fc), "\n")
  
  cat("\nNo Targeted Genes:\n")
  cat("Mean BG signal:", mean(colMeans(no_targeted_bg_profile)), "\n")
  cat("Mean BM signal:", mean(colMeans(no_targeted_bm_profile)), "\n")
  cat("Mean log2 fold change:", mean(no_targeted_fc), "\n")
}

...
#     ${FASTQ_DIR}/BM1_R1_001.fastq.gz \
#     ${FASTQ_DIR}/BM1_R2_001.fastq.gz
...
trimmomatic PE -threads 32 \
    ${FASTQ_DIR}/BM1_R1_001.fastq.gz ${FASTQ_DIR}/BM1_R2_001.fastq.gz \
    ${RESULTS_DIR}/trimmed/BM1_R1_trimmed.fastq.gz ${RESULTS_DIR}/trimmed/BM1_R1_unpaired.fastq.gz \
    ${RESULTS_DIR}/trimmed/BM1_R2_trimmed.fastq.gz ${RESULTS_DIR}/trimmed/BM1_R2_unpaired.fastq.gz \
...
bowtie2 -p 32 \
    ...
    -1 ${RESULTS_DIR}/trimmed/BM1_R1_trimmed.fastq.gz \
    -2 ${RESULTS_DIR}/trimmed/BM1_R2_trimmed.fastq.gz 2> ${RESULTS_DIR}/bowtie2_alt/BM1_align.log | \
    samtools view -h | \
    awk 'BEGIN {OFS="\t"} /^@/ {print} !/^@/ {print $0 "\tRG:Z:BM1"}' | \
    samtools view -q 30 -F 1804 -f 2 -b | \
    samtools sort -@ 32 -o ${RESULTS_DIR}/bowtie2_alt/BM1.sorted.bam -

# 3. Index the BAM file
echo "Indexing BAM file..."
samtools index ${RESULTS_DIR}/bowtie2_alt/BM1.sorted.bam

# 4. Remove PCR duplicates
echo "Removing PCR duplicates..."
picard MarkDuplicates \
    INPUT=${RESULTS_DIR}/bowtie2_alt/BM1.sorted.bam \
...
samtools index ${RESULTS_DIR}/filtered/BM1.dedup.bam
...
zcat ${FASTQ_DIR}/BM1_R1_001.fastq.gz | echo "Raw reads: $((`wc -l`/4))" >> ${RESULTS_DIR}/BM1_qc_metrics.txt

# Count reads after trimming
zcat ${RESULTS_DIR}/trimmed/BM1_R1_trimmed.fastq.gz | echo "After trimming: $((`wc -l`/4))" >> ${RESULTS_DIR}/BM1_qc_metrics.txt

# Get alignment stats
samtools flagstat ${RESULTS_DIR}/bowtie2_alt/BM1.sorted.bam >> ${RESULTS_DIR}/BM1_qc_metrics.txt

...
main <- function() {
  ...
  p1 <- ggplot(plot_data, aes(x = Position, y = Signal, color = Condition, linetype = Group)) +
    ...
    scale_color_manual(values = c("BG" = "yellow", "BM" = "orange"), name = "Condition (BG=GFP, BM=MeCP2)") + # Updated colors
    scale_fill_manual(values = c("BG" = "yellow", "BM" = "orange"), name = "Condition (BG=GFP, BM=MeCP2)") + # Updated colors
    labs(title = "SMARCB1 binding around TSS (Bivalent Genes)",
         subtitle = "Targeted vs. Not Targeted", # Added subtitle for clarity
         x = "Distance from TSS (bp)",
         y = "Average RPKM") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))

  # Create direct comparison plot (BM vs BG for Targeted vs Not Targeted)
  p_direct_comparison <- ggplot(plot_data, aes(x = Position, y = Signal, color = Condition)) +
    geom_line() +
    geom_ribbon(aes(ymin = Signal - SE, ymax = Signal + SE, fill = Condition), alpha = 0.2, color = NA) +
    facet_wrap(~ Group, ncol = 1, scales = "free_y") + # Separate plots for Targeted and Not Targeted
    scale_color_manual(values = c("BG" = "yellow", "BM" = "orange"), name = "Condition (BG=GFP, BM=MeCP2)") +
    scale_fill_manual(values = c("BG" = "yellow", "BM" = "orange"), name = "Condition (BG=GFP, BM=MeCP2)") +
    labs(title = "Direct Comparison: SMARCB1 (BM) vs Control (BG) Signal",
         subtitle = "Bivalent Targeted vs. Not Targeted Genes",
         x = "Distance from TSS (bp)",
         y = "Average RPKM") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5),
  ...
  # Combine plots (p1, p_direct_comparison, p2 vertically)
  combined_plot <- p1 / p_direct_comparison / p2 + plot_layout(heights = c(1, 1.5, 1)) # Adjust heights if needed
  ...
  ggsave(
    ...
    plot = combined_plot,
    width = 10,
    height = 18 # Increased height for three plots
  )

...
#' Compare SMARCB1 binding profiles between non-bivalent targeted and non-targeted genes
#' 
#' Input files:
...
#' - Gene_lists/bivalent/expressed_targeted_non_bivalent_NPCs_1000.csv: List of non-bivalent targeted genes
#' - Gene_lists/bivalent/expressed_not_targeted_non_bivalent_NPCs_1000.csv: List of non-bivalent non-targeted genes
#'
#' Output files:
#' - results/metaprofiles_comparison_R/combined_nonbivalent_targeted_vs_no_targeted_profile.pdf: 
...
#' non-bivalent targeted and non-targeted genes, generating metaprofiles and fold change plots.
...
main <- function() {
  ...
  # Process gene lists
  cat("Processing non-bivalent targeted genes...\n")
  nonbivalent_targeted_regions <- create_tss_regions(nonbivalent_targeted_genes, gtf_file)
  cat("Processing non-bivalent no targeted genes...\n")
  nonbivalent_no_targeted_regions <- create_tss_regions(nonbivalent_no_targeted_genes, gtf_file)
  
  # Calculate profiles
  nonbivalent_targeted_bg_profile <- calculate_metaprofile(bg_data, nonbivalent_targeted_regions)
  nonbivalent_targeted_bm_profile <- calculate_metaprofile(bm_data, nonbivalent_targeted_regions)
  nonbivalent_no_targeted_bg_profile <- calculate_metaprofile(bg_data, nonbivalent_no_targeted_regions)
  nonbivalent_no_targeted_bm_profile <- calculate_metaprofile(bm_data, nonbivalent_no_targeted_regions)
  
  # Calculate means and standard errors
  x_pos <- seq(-2500, 2500, length.out = ncol(nonbivalent_targeted_bg_profile))
  ...
  plot_data <- rbind(
    create_plot_data(nonbivalent_targeted_bg_profile, nonbivalent_targeted_bm_profile, "Non-bivalent Targeted"),
    create_plot_data(nonbivalent_no_targeted_bg_profile, nonbivalent_no_targeted_bm_profile, "Non-bivalent Not Targeted")
  )
  
  # Calculate fold changes
  nonbivalent_targeted_fc <- calculate_fold_change(nonbivalent_targeted_bg_profile, nonbivalent_targeted_bm_profile)
  nonbivalent_no_targeted_fc <- calculate_fold_change(nonbivalent_no_targeted_bg_profile, nonbivalent_no_targeted_bm_profile)
  
  fc_data <- data.frame(
    Position = rep(x_pos, 2),
    FoldChange = c(nonbivalent_targeted_fc, nonbivalent_no_targeted_fc),
  ...
  p1 <- ggplot(plot_data, aes(x = Position, y = Signal, color = Condition, linetype = Group)) +
    ...
    labs(title = "SMARCB1 binding around TSS (Non-bivalent Genes)",
  ...
  p2 <- ggplot(fc_data, aes(x = Position, y = FoldChange, color = Group)) +
    ...
    scale_color_manual(values = c("Non-bivalent Targeted" = "#ff7f0e", "Non-bivalent Not Targeted" = "#2ca02c")) +
  ...
  # Save plots
  ggsave(
    filename = "results/metaprofiles_comparison_R/combined_nonbivalent_targeted_vs_no_targeted_profile.pdf",
    plot = combined_plot,
    width = 10,
    height = 12
  )
  ...
  cat("\nSummary Statistics:\n")
  cat("\nNon-bivalent Targeted Genes:\n")
  cat("Mean BG signal:", mean(colMeans(nonbivalent_targeted_bg_profile)), "\n")
  cat("Mean BM signal:", mean(colMeans(nonbivalent_targeted_bm_profile)), "\n")
  cat("Mean log2 fold change:", mean(nonbivalent_targeted_fc), "\n")
  
  cat("\nNon-bivalent Not Targeted Genes:\n")
  cat("Mean BG signal:", mean(colMeans(nonbivalent_no_targeted_bg_profile)), "\n")
  cat("Mean BM signal:", mean(colMeans(nonbivalent_no_targeted_bm_profile)), "\n")
  cat("Mean log2 fold change:", mean(nonbivalent_no_targeted_fc), "\n")
}

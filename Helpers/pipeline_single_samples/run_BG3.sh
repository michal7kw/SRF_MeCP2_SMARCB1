...
#SBATCH --nodes=1
#SBATCH --ntasks=32
#SBATCH --error="/beegfs/scratch/ric.broccoli/kubacki.michal/Azenta/logs/BG3.err"
#SBATCH --output="/beegfs/scratch/ric.broccoli/kubacki.michal/Azenta/logs/BG3.out"
...
#     ${FASTQ_DIR}/BG3_R1_001.fastq.gz \
#     ${FASTQ_DIR}/BG3_R2_001.fastq.gz
...
trimmomatic PE -threads 32 \
    ${FASTQ_DIR}/BG3_R1_001.fastq.gz ${FASTQ_DIR}/BG3_R2_001.fastq.gz \
    ${RESULTS_DIR}/trimmed/BG3_R1_trimmed.fastq.gz ${RESULTS_DIR}/trimmed/BG3_R1_unpaired.fastq.gz \
    ${RESULTS_DIR}/trimmed/BG3_R2_trimmed.fastq.gz ${RESULTS_DIR}/trimmed/BG3_R2_unpaired.fastq.gz \
...
bowtie2 -p 32 \
    ...
    -1 ${RESULTS_DIR}/trimmed/BG3_R1_trimmed.fastq.gz \
    -2 ${RESULTS_DIR}/trimmed/BG3_R2_trimmed.fastq.gz 2> ${RESULTS_DIR}/bowtie2_alt/BG3_align.log | \
    samtools view -h | \
    awk 'BEGIN {OFS="\t"} /^@/ {print} !/^@/ {print $0 "\tRG:Z:BG3"}' | \
    samtools view -q 30 -F 1804 -f 2 -b | \
    samtools sort -@ 32 -o ${RESULTS_DIR}/bowtie2_alt/BG3.sorted.bam -

# 3. Index the BAM file
echo "Indexing BAM file..."
samtools index ${RESULTS_DIR}/bowtie2_alt/BG3.sorted.bam

# 4. Remove PCR duplicates
echo "Removing PCR duplicates..."
picard MarkDuplicates \
    INPUT=${RESULTS_DIR}/bowtie2_alt/BG3.sorted.bam \
...
samtools index ${RESULTS_DIR}/filtered/BG3.dedup.bam
...
zcat ${FASTQ_DIR}/BG3_R1_001.fastq.gz | echo "Raw reads: $((`wc -l`/4))" >> ${RESULTS_DIR}/BG3_qc_metrics.txt

# Count reads after trimming
zcat ${RESULTS_DIR}/trimmed/BG3_R1_trimmed.fastq.gz | echo "After trimming: $((`wc -l`/4))" >> ${RESULTS_DIR}/BG3_qc_metrics.txt

# Get alignment stats
samtools flagstat ${RESULTS_DIR}/bowtie2_alt/BG3.sorted.bam >> ${RESULTS_DIR}/BG3_qc_metrics.txt

...
create_comparison_plot <- function(plot_data, fc_data, title, subtitle = NULL, group1_n, group2_n) {
  ...
  p1 <- ggplot(plot_data, aes(x = Position, y = Signal, color = Condition, linetype = Group)) +
    ...
    scale_color_manual(values = c("BG" = "#FFD700", "BM" = "#FF6F01")) + # Yellow for BG (GFP?), Orange for BM (MeCP2?)
    scale_fill_manual(values = c("BG" = "#FFD700", "BM" = "#FF6F01")) + # Yellow for BG (GFP?), Orange for BM (MeCP2?)
    labs(title = title,
         subtitle = subtitle,
...
main <- function() {
  ...
  ggsave(filename = file.path(output_dir, "combined_bivalent_vs_nonbivalent_profile.png"), plot = p1, width = 10, height = 12)
  ...
  ggsave(filename = file.path(output_dir, "combined_targeted_vs_no_targeted_profile.png"), plot = p2, width = 10, height = 12)
  ...
  ggsave(filename = file.path(output_dir, "combined_bivalent_targeted_vs_no_targeted_profile.png"), plot = p3, width = 10, height = 12)
  ...
  ggsave(filename = file.path(output_dir, "combined_nonbivalent_targeted_vs_no_targeted_profile.png"), plot = p4, width = 10, height = 12)

...
def plot_heatmaps_bg(bg_targeted: np.ndarray, bg_nontargeted: np.ndarray,
    ...
    combined_data = np.concatenate((bg_targeted.flatten(), bg_nontargeted.flatten()))
    valid_combined_data = combined_data[~np.isnan(combined_data)]

    # Calculate dynamic vmin and vmax using combined data
    if len(valid_combined_data) == 0:
        vmin, vmax = 0, 1
    else:
        min_val = np.min(valid_combined_data[valid_combined_data > 0]) if np.any(valid_combined_data > 0) else 0
        vmin = max(min_val, 1e-6) # Ensure vmin is positive and non-zero if possible
        vmax = np.percentile(valid_combined_data, 99)
        if vmin >= vmax:
            vmax = vmin + 1e-6
...
def plot_heatmaps_bm(bm_targeted: np.ndarray, bm_nontargeted: np.ndarray,
    ...
    # Combine data to find global scale, excluding NaNs
    combined_data = np.concatenate((bm_targeted.flatten(), bm_nontargeted.flatten()))
    valid_combined_data = combined_data[~np.isnan(combined_data)]

...
#' Compare SMARCB1 binding profiles between bivalent and non-bivalent genes
#' 
#' Input files:
...
#'
#' Output files:
#' - results/metaprofiles_comparison_R/combined_bivalent_vs_nonbivalent_profile.pdf: 
#'   Combined plot showing binding profiles and fold changes
#'
#' The script generates metaprofiles comparing SMARCB1 binding around TSS regions
#' between bivalent and non-bivalent genes. It outputs both binding profiles and
...
main <- function() {
  ...
  # Process gene lists
  cat("Processing bivalent genes...\n")
  bivalent_regions <- create_tss_regions(bivalent_genes, gtf_file)
  cat("Processing non-bivalent genes...\n")
  non_bivalent_regions <- create_tss_regions(non_bivalent_genes, gtf_file)
  
  # Calculate profiles
  bivalent_bg_profile <- calculate_metaprofile(bg_data, bivalent_regions)
  bivalent_bm_profile <- calculate_metaprofile(bm_data, bivalent_regions)
  non_bivalent_bg_profile <- calculate_metaprofile(bg_data, non_bivalent_regions)
  non_bivalent_bm_profile <- calculate_metaprofile(bm_data, non_bivalent_regions)
  ...
  plot_data <- rbind(
    create_plot_data(bivalent_bg_profile, bivalent_bm_profile, "Bivalent"),
    create_plot_data(non_bivalent_bg_profile, non_bivalent_bm_profile, "Non-bivalent")
  )
  
  # Calculate fold changes
  bivalent_fc <- calculate_fold_change(bivalent_bg_profile, bivalent_bm_profile)
  non_bivalent_fc <- calculate_fold_change(non_bivalent_bg_profile, non_bivalent_bm_profile)
  
  fc_data <- data.frame(
    Position = rep(x_pos, 2),
    FoldChange = c(bivalent_fc, non_bivalent_fc),
  ...
  # Save plots
  ggsave(
    filename = "results/metaprofiles_comparison_R/combined_bivalent_vs_nonbivalent_profile.pdf",
    plot = combined_plot,
    width = 10,
    height = 12
  )
  ...
  cat("\nSummary Statistics:\n")
  cat("\nBivalent Genes:\n")
  cat("Mean BG signal:", mean(colMeans(bivalent_bg_profile)), "\n")
  cat("Mean BM signal:", mean(colMeans(bivalent_bm_profile)), "\n")
  cat("Mean log2 fold change:", mean(bivalent_fc), "\n")
  
  cat("\nNon-bivalent Genes:\n")
  cat("Mean BG signal:", mean(colMeans(non_bivalent_bg_profile)), "\n")
  cat("Mean BM signal:", mean(colMeans(non_bivalent_bm_profile)), "\n")
  cat("Mean log2 fold change:", mean(non_bivalent_fc), "\n")
}
